...
export class PricingEngine {
  ...
  static calculate({ items = [], promoCode = null, loyaltyPointsUsed = 0, hasCustomization } = {}) {
    const customizedItems = items.filter(i => i && i.customized);
    const applyCustomFee = hasCustomization ?? customizedItems.length > 0;
    ...
    const customizationFee = applyCustomFee
      ? PricingEngine.CUSTOMIZATION_FEE * customizedItems.length
      : 0;

    let deliveryFee = subtotal >= PricingEngine.FREE_SHIPPING_THRESHOLD ? 0 : PricingEngine.SHIPPING_BASE;
    let promoDiscount = 0;
    let promoLabel = '';
    ...
    const taxableAmount = Math.max(0, subtotal + customizationFee - promoDiscount - loyaltyDiscount);
  ...
  static validatePromo(code, orderTotal) {
    ...
    let promo;
    try {
      promo = db.get(`SELECT * FROM promo_codes WHERE code = ? AND is_active = 1`, [code.toUpperCase()]);
    } catch (err) {
