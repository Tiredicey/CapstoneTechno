...
router.post('/', optionalAuth, (req, res) => {
  try {
    const {
      cartId, recipient, deliveryDate, deliverySlot,
      recurring, paymentMethod, specialInstructions, loyaltyPointsUsed
    } = req.body;

    if (!recipient || !deliveryDate || !deliverySlot || !paymentMethod) {
    ...
    items.forEach(item => {
      if (item.productId) ProductModel.decrementInventory(item.productId, item.qty || 1);
    });
...
router.get('/my', authenticate, (req, res) => {
  try {
    res.json(OrderModel.getByUser(req.user.id));
  } catch (err) {
...
router.put('/:id/florist', authenticate, requireAdmin, (req, res) => {
  try {
    OrderModel.assignFlorist(req.params.id, req.body.floristId);
    res.json({ ok: true });
  } catch (err) {
    console.error('[FLORIST ERROR]', err);
    res.status(500).json({ error: 'Failed to assign florist' });
  }
});
