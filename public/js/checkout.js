...
async function init() {
  ...
  if (sub && subPrice) {
    ...
    try {
      await Api.post('/api/cart/items', { productId: 'sub-' + sub, price: subPrice, qty: 1 });
    } catch (e) {
      console.warn('[Checkout] Failed to sync subscription to server:', e.message);
    }
    try {
      var serverCart = await Api.get('/api/cart');
      cartData = serverCart && serverCart.items && serverCart.items.length ? serverCart : {
        items: [{
          productId: 'sub-' + sub,
        ...
        pricing: { subtotal: Number(subPrice), customizationFee: 0, deliveryFee: 0, tax: 0, finalTotal: Number(subPrice), promoDiscount: 0 }
      };
    } catch (e) {
      cartData = {
        items: [{ productId: 'sub-' + sub, name: sub.charAt(0).toUpperCase() + sub.slice(1) + ' Subscription', price: Number(subPrice), qty: 1, image: '/uploads/subscriptions/' + sub + '.jpg' }],
        pricing: { subtotal: Number(subPrice), customizationFee: 0, deliveryFee: 0, tax: 0, finalTotal: Number(subPrice), promoDiscount: 0 }
      };
    }
    ...
    try {
      ...
      if (!cartData || !cartData.items || !cartData.items.length) {
        try {
          ...
          if (local.length) {
            await Promise.all(local.map(function (it) {
              return Api.post('/api/cart/items', it).catch(function (err) {
                console.warn('[Checkout] Rehydrate item failed:', it && it.productId, err && err.message);
              });
            }));
...
function renderCheckoutSummary() {
  ...
  if (itemsEl) {
    itemsEl.innerHTML = cartData.items
      .map(
        (i) =>
          `      <div style="display:flex;gap:12px;align-items:center;font-size:0.83rem;">        <div style="width:40px;height:40px;border-radius:8px;background:rgba(139,31,110,0.2);display:flex;align-items:center;justify-content:center;flex-shrink:0;">\uD83C\uDF38</div>        <div style="flex:1;">${i.name}<br><span style="color:rgba(255,255,255,0.4);">\u00D7${i.qty || 1}</span></div>        <div style="font-weight:600;color:#FFD700;">\u20B1${(i.price * (i.qty || 1)).toFixed(2)}</div>      </div>`
      )
      .join('');
  }
}
...
function showToast(msg, type = 'info') {
  ...
  toast.style.cssText = `    position:fixed;bottom:24px;right:24px;z-index:99999;    background:${type === 'success' ? '#22c55e' : type === 'error' ? '#ef4444' : '#6366f1'};    color:white;padding:12px 24px;border-radius:12px;    font-size:14px;font-weight:600;box-shadow:0 4px 20px rgba(0,0,0,0.3);  `;
...
document.getElementById('placeOrder')?.addEventListener('click', async () => {
  ...
  try {
    ...
    const isSurprise = document.getElementById('surpriseDelivery')?.checked || false;
    const payload = {
      cartId,
      recipient: {
